...
fn migrate_world_without_seed_will_fail() {
    ...
    let manifest = World::load_from_path(target_dir.join("manifest.json")).unwrap();
...
async fn migration_from_remote() {
    ...
    let local_manifest = World::load_from_path(target_dir.join("manifest.json")).unwrap();
    let remote_manifest = World::load_from_remote(
        JsonRpcClient::new(HttpTransport::new(sequencer.url())),
