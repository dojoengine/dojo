...
impl DatabaseBackend for SqliteBackend {
    ...
    async fn set_head(
        ...
        let last_block_timestamp_arg = Argument::Int(
            last_block_timestamp
                .try_into()
                .map_err(|_| anyhow!("Last block timestamp value {} doesn't fit in i64", last_block_timestamp))?,
        );
    ...
    async fn cursors(&self) -> Result<Cursors> {
        ...
        let last_pending_block_tx = last_pending_block_tx
            .map(|t| Felt::from_str(&t).expect("its a valid felt"));
    ...
    fn set_metadata(&mut self, resource: &Felt, uri: &str, block_timestamp: u64) -> Result<()> {
        ...
        self.executor.send(QueryMessage::other(
            "INSERT INTO metadata (id, uri, executed_at) VALUES (?, ?, ?) \
             ON CONFLICT(id) DO UPDATE SET id=excluded.id, executed_at=excluded.executed_at, \
    ...
    async fn entities(&self, model: String) -> Result<Vec<Vec<Felt>>> {
        let query = sqlx::query_as::<_, (i32, String, String)>("SELECT * FROM ?")
            .bind(model);
        let mut conn = self.pool.acquire().await?;
        let mut rows = query.fetch_all(&mut *conn).await?;
        Ok(rows
            .drain(..)
            .map(|row| serde_json::from_str(&row.2).unwrap())
            .collect())
    }
    ...
    fn store_transaction(
        ...
        self.executor.send(QueryMessage::other(
            "INSERT OR IGNORE INTO transactions \
             ...
             transaction_type, executed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                .to_string(),
            vec![
                id,
                transaction_hash,
    ...
    fn store_event(
        ...
        self.executor.send(QueryMessage::new(
            "INSERT OR IGNORE INTO events (id, keys, data, transaction_hash, executed_at) \
             VALUES (?, ?, ?, ?, ?) RETURNING *"
                .to_string(),
