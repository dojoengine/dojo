...
use crate::processors::NUM_KEYS_INDEX;
use crate::processors::{BlockProcessor, EventProcessor, TransactionProcessor, ENTITY_ID_INDEX};
use crate::sql::Sql;
...
impl<P: Provider + Send + Sync + std::fmt::Debug + Clone + 'static> Engine<P> {
    ...
    pub async fn process_pending(&mut self, data: FetchPendingResult) -> Result<()> {
        ...
        let mut map = HashMap::<u8, Vec<(Event, Felt, u64, u64)>>::new();
        let modulo = 8u64;
        let other = modulo + 1;
            ...
            for event in events {
                ...
                match event_name {
                    EventType::StoreSetRecord => {
                        ...
                        let entity_id = entity_id.to_raw()[3] % modulo;
                    ...
                    EventType::Other => {
                        map.entry(other as u8).or_default().push((
                            event,
                            *transaction_hash,
                            block_number,
                            block_timestamp,
    ...
    pub async fn process_range(&mut self, data: FetchRangeResult) -> Result<()> {
        ...
        let modulo = 8u64;
        let other = modulo + 1;
        let mut map = HashMap::<u8, Vec<(EmittedEvent, u64, u64)>>::new();
...
fn event_type_from_felt(input: Felt) -> EventType {
    ...
    let store_delete = selector!("StoreDeleteRecord");
    ...
    match input {
        ...
        x if x == store_delete => EventType::StoreDeleteRecord,
