...
mod tests {
    ...
    use dojo_test_utils::{compiler::build_test_config, migration::prepare_migration, sequencer::{get_default_test_starknet_config, SequencerConfig, TestSequencer}};
    use dojo_world::{contracts::WorldContractReader, utils::TransactionWaiter};
    use scarb::ops;
    use sozo_ops::migration::execute_strategy;
    use sqlx::sqlite::{SqliteConnectOptions, SqlitePoolOptions};
    use starknet::{accounts::{Account, Call}, core::{types::{BlockId, BlockTag}, utils::get_selector_from_name}, providers::{jsonrpc::HttpTransport, JsonRpcClient}};
    use tokio::sync::broadcast;
    use torii_core::{engine::{Engine, EngineConfig, Processors}, processors::{register_model::RegisterModelProcessor, store_set_record::StoreSetRecordProcessor}, sql::Sql};
    ...
    async fn test_entities_queries() {
        ...
        let grpc = DojoWorld::new(db.pool, receiver, migration.world_address.unwrap(), provider.clone());
