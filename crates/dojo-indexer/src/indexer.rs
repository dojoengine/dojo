...
pub struct BlockIngestion<'a, DB: Database, T: JsonRpcTransport> {
    head: u64,
    provider: &'a JsonRpcClient<T>,
    db: &'a Pool<DB>,
}

impl<'a, DB: Database, T: JsonRpcTransport> BlockIngestion<'a, DB, T> {
    pub fn new(provider: &'a JsonRpcClient<T>, db: &'a Pool<DB>) -> Self {
        BlockIngestion { head: 0, provider, db }
    }

    pub async fn start(self, ct: CancellationToken) -> Result<(), BlockIngestionError> {
        let mut conn =
            self.db.acquire().await.map_err(|err| BlockIngestionError::DatabaseError(err))?;
        let indexer = sqlx::query!("SELECT head FROM indexer WHERE id = 0",)
            .fetch_one(&mut conn)
            .await
            .map_err(|err| BlockIngestionError::DatabaseError(err))?;

        // loop {
        //     let latest_indexed = match self.head {
        //         Some(block) => block,
...
pub async fn start_indexer<DB: Database, T: JsonRpcTransport + Sync + Send>(
