...
use super::{
    entity_state::EntityState, entity_state_update::EntityStateUpdate, system::System, Query, entity::Entity
};
    ...
    async fn component(context: &PrismaClient, id: String) -> Option<Component> {
        ...
        match component {
            Some(component) => Some(Component {
                ...
                systems: component.clone().systems.unwrap().into_iter().map(|system| System {
                    id: system.id,
                    name: system.name,
                ...
                states: component.clone().states.unwrap().into_iter().map(|state| EntityState {
                    data: state.data,
                    entity: Entity {
                ...
                state_updates: component.clone().state_updates.unwrap().into_iter().map(|state_update| EntityStateUpdate {
                    id: state_update.id,
