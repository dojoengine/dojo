...
#[test]
#[available_gas(30000000)]
#[should_panic(expected: ('invalid token_id', 'ENTRYPOINT_FAILED'))]
fn test_uri_with_invalid_id() {
    let (world, erc721) = deploy_default();
    erc721.token_uri(0_u256); // should panic
}
...
fn test_mint_simple() {
    ...
    erc721.mint(user1(), 0_u256);
    erc721.mint(user2(), 1_u256);
    erc721.mint(user2(), 2_u256);

    assert(erc721.balance_of(user1()) == 1_u256, 'balance should be 1');
    assert(erc721.balance_of(user2()) == 2_u256, 'balance should be 2');
...
fn test_mint_same_id_twice() {
    ...
    erc721.mint(user1(), 0_u256);
    erc721.mint(user2(), 0_u256); // should panic
}
...
fn test_burn_simple() {
    ...
    assert(erc721.balance_of(user1()) == 0_u256, 'balance should be 0');
...
fn test_burn_invalid_id() {
    let (world, erc721) = deploy_default();
    erc721.burn(0_u256); // should panic
}
