...
impl Library {
    pub fn from_module(db: &dyn SyntaxGroup, module_ast: ast::ItemModule) -> PluginResult {
        ...
        let mut library = Library { diagnostics: vec![], dependencies: HashMap::new() };
            ...
            let body_nodes = body
                ...
                .flat_map(|el| {
                    if let ast::Item::FreeFunction(fn_ast) = el {
                        return library.from_function(db, fn_ast.clone());
                    }
                ...
                code: Some(PluginGeneratedFile {
                    ...
                    aux_data: DynGeneratedFileAuxData::new(DynPluginAuxData::new(DojoAuxData {
                        ...
                        systems: vec![SystemAuxData {
                            name,
                            dependencies: library.dependencies.values().cloned().collect(),
                        }],
                    })),
                }),
