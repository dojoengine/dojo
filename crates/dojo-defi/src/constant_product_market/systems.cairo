...
mod Buy {
    ...
    fn execute(partition: felt252, item_id: felt252, quantity: usize) {
        ...
        let cash_sk: Query = (partition, (player)).to_key();
        let player_cash = commands::<Cash>::entity(cash_sk);

        let market_sk: Query = (partition, (item_id)).to_key();
        ...
        // update player item
        let item_sk: Query = (partition, (player, item_id)).to_key();
...
mod AddLiquidity {
    ...
    fn execute(partition: felt252, item_id: felt252, amount: u128, quantity: usize) {
        ...
        let liquidity_sk: Query = (partition, (player, item_id)).to_key();
