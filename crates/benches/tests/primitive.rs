...
use benches::{deploy_sync, estimate_gas, log, runner, BenchCall, Felt};
#[cfg(not(feature = "skip-gas-benchmarks"))]
...
proptest! {
    ...
    fn bench_primitive_iter(s in 990..1010) {
        ...
        let s = format!("{}", s);
        let s_hex = Felt::from_dec_str(&s).unwrap();
    ...
    fn bench_primitive_hash(a in 0..u64::MAX, b in 0..u64::MAX, c in 0..u64::MAX) {
        ...
        let b = format!("{}", b);
        let c = format!("{}", c);
        let args = vec![a.clone(), b.clone(), c.clone()].into_iter().map(|d| Felt::from_dec_str(&d).unwrap()).collect::<Vec<_>>();
