    ...
    for crate_id in crate_ids {
        ...
        for module_id in modules.iter() {
            ...
            for generated_file_info in generated_file_infos.iter().skip(1) {
                ...
                for component_name in &aux_data.components {
                    if let Ok(Some(ModuleItemId::Submodule(submodule_id))) =
                        db.module_item_by_name(*module_id, component_name.clone())
                    {
                        components.push(ComponentDeclaration { submodule_id });
                    } else {
                        panic!("Component `{component_name}` was not found.");
                    }
                }
            }
        }
