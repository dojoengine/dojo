...
impl UnrealEnginePlugin {
    ...
    fn map_type(token: &Token) -> String {
        match token.type_name().as_str() {
            ...
            //             .inners
            //             .iter()
            //             .map(UnrealEnginePlugin::map_type)
            //             .collect::<Vec<String>>()
            //             .join(", ");
            //         format!("({})", inners)
            //     } else {
...
public:
    ...
    fn handle_cppfile(
        ...
        {{
            ParsedModel = AGeneratedHelpers::parse{namespaceCamelCase}{modelName}Model(&models->data[Index]);
        }}",
...
", parse_models_functions_bodies, if_else_parse_models);
        ...
        let mut functions_bodies: String = String::new();
        for (_, contract) in contracts {
            ...
            for system_token in contract.systems.iter().filter(|s| {
                ...
                let args_body = if system_token.to_function().unwrap().inputs.is_empty() {
                    ...
                    let args_text = system_token
                        ...
                        .map(|arg| {
                            format!(
                                "    arg = DojoModule::parseType({name}, \"{arg_type}\")
    if (arg.Size() <= 0) {{
        arg = this->parseCustomType{arg_type}({name})
    }}
    if (!args.IsEmpty()) {{
        args += TEXT(\", \");
    }}
...
",
                    ...
                    contract_name = format!("{}-{}", naming::get_namespace_from_tag(&contract.tag), naming::get_name_from_tag(&contract.tag)),
                    args = if args.is_empty() { String::from("") } else { format!(", {}", args) },
                    args_body = args_body,
                );
            }
...
impl BuiltinPlugin for UnrealEnginePlugin {
    async fn generate_code(&self, data: &DojoData) -> BindgenResult<HashMap<PathBuf, Vec<u8>>> {
        ...
        let header_path = Path::new(&format!("GeneratedHelpers.h")).to_owned();
        let code = self.handle_header(&models, &contracts);
        out.insert(header_path, code.as_bytes().to_vec());

        println!("Generating header: GeneratedHelpers.cpp");
        let cppfile_path = Path::new(&format!("GeneratedHelpers.cpp")).to_owned();
        let code = self.handle_cppfile(&models, &contracts);
        out.insert(cppfile_path, code.as_bytes().to_vec());
