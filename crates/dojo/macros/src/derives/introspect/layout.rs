...
pub fn build_array_layout_from_type(diagnostics: &mut Vec<Diagnostic>, item_type: &str) -> String {
    ...
    if is_tuple(&array_item_type) {
        format!(
            ...
            build_item_layout_from_type(diagnostics, &array_item_type)
        )
        ...
        format!(
            ...
            build_array_layout_from_type(diagnostics, &array_item_type)
        )
    } else {
...
pub fn build_tuple_layout_from_type(diagnostics: &mut Vec<Diagnostic>, item_type: &str) -> String {
    let tuple_items = get_tuple_item_types(item_type)
        .iter()
        .map(|x| build_item_layout_from_type(diagnostics, x))
        .collect::<Vec<_>>()
        .join(",\n");
    format!(
...
pub fn get_packed_tuple_layout_from_type(
    ...
    get_tuple_item_types(item_type)
        .iter()
        .flat_map(|x| get_packed_item_layout_from_type(diagnostics, x))
        .collect::<Vec<_>>()
}
