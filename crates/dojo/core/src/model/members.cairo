...
pub impl MemberStoreImpl<
    S, M, T, +Model<M>, +MemberModelStorage<S, M, T>, +ModelDefinition<M>, +Serde<T>
> of MemberStore<S, M, T> {
    fn get_member(self: @S, entity_id: felt252, member_id: felt252) -> T {
        MemberModelStorage::<S, M, T>::read_member(self, entity_id, member_id)
    }

    fn update_member(ref self: S, entity_id: felt252, member_id: felt252, value: T) {
        MemberModelStorage::<S, M, T>::write_member(ref self, entity_id, member_id, value)
    }
}
