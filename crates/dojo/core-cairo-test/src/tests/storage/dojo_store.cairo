...
fn test_dojo_store_primitives() {
    ...
    // ContractAddress
    let mut serialized = array![];
    DojoStore::serialize(@core::starknet::contract_address_const::<1>(), ref serialized);
    assert_eq!(serialized, array![1], "DojoStore<ContractAddress> serialization failed");

    let mut values = [1].span();
    let res = DojoStore::<core::starknet::ContractAddress>::deserialize(ref values);
    assert_eq!(
        res,
        Option::Some(core::starknet::contract_address_const::<1>()),
        "DojoStore<ContractAddress> deserialization failed",
    );

    // ClassHash
    let mut serialized = array![];
    DojoStore::serialize(@core::starknet::class_hash::class_hash_const::<1>(), ref serialized);
    assert_eq!(serialized, array![1], "DojoStore<ClassHash> serialization failed");

    let mut values = [1].span();
    let res = DojoStore::<core::starknet::ClassHash>::deserialize(ref values);
    assert_eq!(
        res,
        Option::Some(core::starknet::class_hash::class_hash_const::<1>()),
        "DojoStore<ClassHash> deserialization failed",
    );

    // EthAddress
    let eth_address: core::starknet::EthAddress = 1.try_into().unwrap();
    ...
    let mut values = [1].span();
    let res = DojoStore::<core::starknet::EthAddress>::deserialize(ref values);
