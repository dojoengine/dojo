...
fn test_storage() {
    ...
    storage::set_many(0, keys.span(), values.span(), 0, layout);
    let res = storage::get_many(0, keys.span(), layout);
...
fn test_storage_empty() {
    ...
    let many = storage::get_many(0, keys.span(), [251, 251, 251].span());
    ...
    let many = storage::get_many(0, keys.span(), [8, 8, 32].span());
    assert(many.len() == 0x3, 'Array should be len 3');
    assert((*many[0]) == 0x0, 'Array[0] should be 0');
    assert((*many[1]) == 0x0, 'Array[1] should be 0');
    assert((*many[2]) == 0x0, 'Array[2] should be 0');
...
fn test_storage_set_many() {
    ...
    storage::set_many(0, keys.span(), values.span(), 0, [251, 251, 251, 251].span());
    let many = storage::get_many(0, keys.span(), [251, 251, 251, 251].span());
...
fn test_storage_set_many_several_segments() {
    ...
    storage::set_many(0, keys.span(), values.span(), 0, layout.span());
    let many = storage::get_many(0, keys.span(), layout.span());
