...
fn test_database_pagination() {
    // TODO: test very huge array that occupies several storage segments.
    // let mut values = ArrayTrait::new();
    // values.append(0x1);
    // values.append(0x2);
    // values.append(0x3);
...
fn test_database_del() {
    ...
    set('table', 'key', values.span(), array![251].span());

    let before = get('table', 'key', array![251].span());
    assert(*before.at(0) == *values.at(0), 'Values different at index 0!');

    del('table', 'key');
    let after = get('table', 'key', array![].span());
