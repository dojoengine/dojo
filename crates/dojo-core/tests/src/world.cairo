...
fn test_component() {
    ...
    World::set_entity(ctx, name, KeyTrait::new_from_column(id.into()), 0, data.span());
    let stored = World::entity(name, KeyTrait::new_from_column(id.into()), 0, 1);
...
fn test_assume_role() {
    ...
    systems.append('Bar');
    world.assume_role('FooWriter', systems);

    // Get execution role
    let role = world.execution_role();
    assert(role == 'FooWriter', 'role not assumed');

    // Get systems for execution
    let is_system_for_execution = world.is_system_for_execution('Bar');
...
fn test_initialize() {
    ...
    let role = world.entity('AuthRole', (target_id, resource_id).into(), 0, 0);
    assert(*role[0] == 'FooWriter', 'role not stored');

    // Assert that the status is stored
    let status = world.entity('AuthStatus', (role_id, resource_id).into(), 0, 0);
    assert(*status[0] == 1, 'status not stored');

    let is_authorized = world.is_authorized('Bar', 'Foo', AuthRole { id: role_id });
...
fn test_set_entity_authorized_with_assumed_role() {
    ...
    let foo = world.entity('Foo', world_address.into(), 0, 0);
...
fn test_admin_system_but_non_admin_caller() {
    ...
    world.execute('RevokeAuthRole', grant_role_calldata.span());
    let role = world.entity('AuthRole', KeyTrait::new_from_column(caller), 0, 0);
...
fn test_set_entity_directly() {
    ...
    data.append(420);
    data.append(1337);
    world.set_entity(ctx, 'Foo', KeyTrait::new_from_column(id.into()), 0, data.span());
}
...
fn test_grant_role() {
    ...
    let role = world.entity('AuthRole', 'Bar'.into(), 0, 0);
...
fn test_revoke_role() {
    ...
    world.execute('RevokeAuthRole', revoke_role_calldata.span());
...
fn test_grant_scoped_role() {
    ...
    world.execute('GrantScopedAuthRole', grant_role_calldata.span());
...
fn test_revoke_scoped_role() {
    ...
    world.execute('RevokeScopedAuthRole', revoke_role_calldata.span());
...
fn test_grant_resource() {
    ...
    grant_role_calldata.append('FooWriter'); // role_id
    grant_role_calldata.append('Foo'); // resource_id
    world.execute('GrantResource', grant_role_calldata.span());

    // Assert that the access is set
    let status = world.entity('AuthStatus', ('FooWriter', 'Foo').into(), 0, 0);
...
fn test_revoke_resource() {
    ...
    revoke_role_calldata.append('Foo'); // resource_id
    world.execute('RevokeResource', revoke_role_calldata.span());
...
fn test_assume_admin_role_by_non_admin() {
    ...
    grant_role_calldata.append(caller); // target_id
    world.execute('RevokeAuthRole', grant_role_calldata.span());
    let role = world.entity('AuthRole', KeyTrait::new_from_column(caller.into()), 0, 0);
    assert(*role[0] == 0, 'role not revoked');

    // Non-admin assume Bar system that has Admin role
    // Should panic since caller is not admin anymore
    let mut systems = ArrayTrait::<felt252>::new();
    world.assume_role('Bar', systems);
}
...
fn test_library_call_system() {
    ...
    calldata.append(0);
    world.execute('LibraryCall', calldata.span());
}
