...
// OpenZeppelin Contracts for Cairo v0.17.0 (presets/erc20.cairo)
...
pub mod AppchainFeeToken {
	...
	fn initialize(
    	...
    	let name = Serde::deserialize(ref serialized).unwrap();
    	let symbol = Serde::deserialize(ref serialized).unwrap();
    	let fixed_supply = Serde::deserialize(ref serialized).unwrap();
    	let recipient = Serde::deserialize(ref serialized).unwrap();
    	let owner = Serde::deserialize(ref serialized).unwrap();
	...
	#[external(v0)]
	fn initialized(self: @ContractState) -> bool {
