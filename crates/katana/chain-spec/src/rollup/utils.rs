...
use katana_primitives::genesis::constant::{
    DEFAULT_ACCOUNT_CLASS, DEFAULT_ACCOUNT_CLASS_HASH, DEFAULT_LEGACY_UDC_CLASS,
    DEFAULT_STRK_FEE_TOKEN_ADDRESS, GENESIS_ACCOUNT_CLASS,
};
...
/// The contract address of the fee token generated by [`GenesisTransactionsBuilder`].
///
/// See [`GenesisTransactionsBuilder::deploy_token_contract`]
pub const DEFAULT_APPCHAIN_FEE_TOKEN_ADDRESS: ContractAddress = DEFAULT_STRK_FEE_TOKEN_ADDRESS;
...
impl<'c> GenesisTransactionsBuilder<'c> {
    ...
    // This is done so that we get the same contract address for the token contract as Starknet.
    fn deploy_token_contract(&mut self) -> ContractAddress {
        // Proxy class deployment as seen on Starknet https://voyager.online/tx/0x4b14b46ecadb7b16d626b1258e69c9a372d49f0280f35824619d830aea257d2
        let proxy_class =
            ContractClass::from_str(include_str!("contracts/strk_init_proxy.json")).unwrap();

        let proxy_class_hash = self.legacy_declare(proxy_class);
        let contract_address_salt =
            felt!("0x048a38cb46716a7cc3a7b5132309388f298bc49c53f5de377bb5401d877b7f89");

        let proxy_address =
            self.deploy(dbg!(proxy_class_hash), vec![Felt::ZERO], contract_address_salt);
        dbg!(proxy_address);

        let fee_token_class =
            ContractClass::from_str(include_str!("contracts/AppchainFeeToken.json")).unwrap();
        let fee_token_class_hash = self.declare(fee_token_class);

        let master_address = *self.master_address.get().expect("must be initialized first");

        let name = ByteArray::cairo_serialize(&"Starknet Token".try_into().unwrap());
        let symbol = ByteArray::cairo_serialize(&"STRK".try_into().unwrap());

        let length = name.len() + symbol.len() + 4;
        let init_vector = iter::once(fee_token_class_hash)
            .chain(iter::once(Felt::ZERO))
            .chain(iter::once(Felt::from(length)))
            .chain(name)
            .chain(symbol)
            .chain(iter::once(Felt::from_u128(u128::MAX).unwrap()))
            .chain(iter::once(Felt::from_u128(u128::MAX).unwrap()))
            .chain(iter::once(master_address.into()))
            .chain(iter::once(master_address.into()))
            .chain(iter::once(Felt::ZERO))
            .collect::<Vec<_>>();

        self.invoke(proxy_address, "add_implementation", init_vector.clone());
        self.invoke(proxy_address, "upgrade_to", init_vector);
        self.fee_token.set(proxy_address).expect("must be uninitialized");
    ...
    fn build_core_contracts(&mut self) {
        ...
        let _ = self.deploy(udc_class_hash, Vec::new(), Felt::ZERO);
        let _ = self.deploy_token_contract();
    }
