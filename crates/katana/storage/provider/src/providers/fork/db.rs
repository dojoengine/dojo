...
use super::backend::ForkedBackendHandler;
...
pub struct ForkedProvider {
    ...
    backend: Arc<ForkedBackendHandler>,
}

impl ForkedProvider {
    /// Construct a new `ForkedDb` from a `Provider` of the network to fork from at a particular
    /// `block`.
    pub fn new(provider: Arc<JsonRpcClient<HttpTransport>>, block: BlockHashOrNumber) -> Self {
        Self {
            storage: InMemoryProvider::new(),
            backend: Arc::new(ForkedBackendHandler::new_with_backend_thread(provider, block)),
        }
    }
}

// impl State for ForkedProvider {
//     fn set_storage_at(
//         &mut self,
...
// impl StateReader for ForkedProvider {
...
// StateResult<CompiledClassHash> {         self.db.get_compiled_class_hash(class_hash)
//     }
...
// impl AsStateRefDb for ForkedProvider {
//     fn as_ref_db(&self) -> StateRefDb {
//         StateRefDb::new(self.clone())
//     }
// }

// impl MaybeAsCachedDb for ForkedProvider {
...
// impl Database for ForkedProvider {
...
//         let provider =
// JsonRpcClient::new(HttpTransport::new(Url::parse(FORKED_ENDPOINT).unwrap()));         let mut
// cache = CachedDb::new(SharedBackend::new_with_backend_thread(             Arc::new(provider),
...
//         let mut db = ForkedProvider::new_from_backend(cache);
...
//         let provider =
// JsonRpcClient::new(HttpTransport::new(Url::parse(FORKED_ENDPOINT).unwrap()));         let mut db
// = ForkedProvider::new(Arc::new(provider), BlockId::Tag(BlockTag::Latest));
...
//         assert_eq!(class_hash_in_cache, expected_class_hash, "class hash must be stored in
// cache");         assert_eq!(storage_value_in_cache, storage_value, "storage value must be stored
// in cache");     }
// }
