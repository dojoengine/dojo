...
//
// the executor must wrap the pending state so that it can perform validation against the pending
// block state. this validator only validate the tx validation and doesn't take into consideration
// of its nonce.
...
//         &self,
//         tx: Self::Tx,
//     ) -> Result<ValidationResult<Self::Tx>, Box<dyn std::error::Error>> {
//         let result = self.inner.lock().perform_validations(todo!(), None);
//         Ok(ValidationResult::Valid { tx })
//     }
// }
