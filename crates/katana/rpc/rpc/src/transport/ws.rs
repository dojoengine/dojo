            ...
            .filter_map(|v| {
                if let Ok(req) = serde_json::from_str::<Request>(v.get()) {
                    Some(Either::Right(async {
                        execute_call(req, call.clone()).await.into_inner()
                    }))
                } else if let Ok(_notif) =
                    serde_json::from_str::<Notification<&JsonRawValue>>(v.get())
                {
...
pub(crate) async fn execute_call<'a, L: Logger>(
    req: Request<'a>,
    call: CallData<'_, L>,
) -> MethodResult {
    let CallData {
                ...
                match method.claim(name, resources) {
                    Ok(guard) => {
                        if let Some(cn) = bounded_subscriptions.acquire() {
                            ...
                            let response =
                                callback(id.clone(), params, sink.clone(), conn_state, Some(guard))
                                    .await;
...
pub(crate) async fn background_task<L: Logger>(
    ...
    let result = loop {
...

        {
            ...
            if let Err(err) =
                method_executors.select_with(Monitored::new(receive, &stop_handle)).await
            {
                match err {
                    MonitoredError::Selector(SokettoError::Closed) => {
                        tracing::debug!(
                            "WS transport: remote peer terminated the connection: {}",
                            conn_id
                        );
                        break Ok(());
                    }
                    MonitoredError::Selector(SokettoError::MessageTooLarge {
                        current,
                        maximum,
                    }) => {
                        tracing::warn!(
