...
var basicSizePlusOne = new BN2(basicAlphabet.length + 1);
var bigAlphabet = "\u8FD9\u6765";
var basicAlphabetSize = new BN2(basicAlphabet.length);
var bigAlphabetSize = new BN2(bigAlphabet.length);
var bigAlphabetSizePlusOne = new BN2(bigAlphabet.length + 1);
...
function useEncoded(decoded) {
  let encoded = new BN2(0);
  let multiplier = new BN2(1);
  ...
  for (let i = 0; i < decoded.length; i += 1) {
    ...
    const bnIndex = new BN2(basicAlphabet.indexOf(char));
    if (index !== -1) {
      ...
      encoded = encoded.add(multiplier.mul(new BN2(newid)));
...
async function getStarkName(provider, address, StarknetIdContract) {
  ...
  try {
    ...
    const decimalDomain = hexDomain.result.map((element) => new BN3(hexToDecimalString(element))).slice(1);
...
var CheckCallData = class {
  ...
  validateMethodAndArgs(type, method, args = []) {
    ...
    methodAbi.inputs.forEach((input) => {
      ...
      if (input.type === "felt") {
        assert4(
          typeof args[argPosition] === "string" || typeof args[argPosition] === "number" || args[argPosition] instanceof BN5,
        ...
        if (input.type === "felt*") {
          args[argPosition].forEach((felt) => {
            assert4(
              typeof felt === "string" || typeof felt === "number" || felt instanceof BN5,
